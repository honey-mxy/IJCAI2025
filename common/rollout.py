...
class CommRolloutWorker:
    ...
    def generate_episode(self, episode_num=None, evaluate=False):
        ...
        while not terminated and step < self.episode_limit:
            ...
            weights, hard_weights = self.agents.get_action_weights(np.array(obs), last_action)
            # print(weights)
            #print(int(hard_weights[0].tolist()[0][0][0]))

            # choose action for each agent
            for agent_id in range(self.n_agents):
                avail_action = self.env.get_avail_agent_actions(agent_id)
                #action = self.agents.choose_action(weights[agent_id], avail_action, epsilon)
                action = self.agents.choose_action(weights[agent_id], avail_action, 0)
            ...
            edge_left = 0
            for i in range(0,3):
                print(hard_weights[i])
                for j in range(0,2):
                    if int(hard_weights[i].tolist()[0][0][j]) == 1:
                        edge_left += 1

            reward, terminated, info = self.env.step(actions)
            reward = (0 - reward) / 100 - (6 - edge_left)
            win_tag = True if terminated and 'battle_won' in info and info['battle_won'] else False
            o.append(obs)
            s.append(state)
            u.append(np.reshape(actions, [self.n_agents, 1]))
