...
class CommAgents:
    def __init__(self, args):
        ...
        if alg.find('reinforce') > -1:
            from policy.reinforce import Reinforce
            self.policy = Reinforce(args, is_attack = False)
            self.policy_attack = Reinforce(args, is_attack = False)
            #self.policy_old = Reinforce(args)
    ...
    def get_action_weights(self, obs, last_action):
        ...
        weights, self.policy.eval_hidden, hard_weights = self.policy.eval_rnn(inputs, self.policy.eval_hidden)
        weights = weights.reshape(self.args.n_agents, self.args.n_actions)
        return weights.cpu(), hard_weights
    ...
    def train(self, batch, train_step, epsilon=None):  # coma在训练时也需要epsilon计算动作的执行概率
        ...
        for key in batch.keys():
            batch[key] = batch[key][:, :max_episode_len]
        self.policy_attack.learn(batch, max_episode_len, train_step, epsilon)
        if train_step > 0 and train_step % self.args.save_cycle == 0:
            self.policy_attack.save_model(train_step)
