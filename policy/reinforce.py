...
class Reinforce:
    def __init__(self, args, is_attack):
        ...
        self.model_dir = args.pretrain_dir + '/' + args.alg + '/' + args.map
        # 如果存在模型则加载模型
        if self.args.load_model:
            if os.path.exists(self.model_dir + '/1_rnn_params.pkl'):
                path_rnn = self.model_dir + '/1_rnn_params.pkl'
    ...
    def _get_action_prob(self, batch, max_episode_len, epsilon):
        ...
        for transition_idx in range(max_episode_len):
            ...
            outputs, self.eval_hidden,hard_weights = self.eval_rnn(inputs, self.eval_hidden)
